...
void MyBank::Trata(int N) {
    ...
    if (tempBalance > BALANCE[N]) {
        ...
        BALANCE[N] -= tempBalance;
        char* temp = new char;//дл€ ввода новой категории
        int cat;//дл€ выбора к какой категории отнести
...
void MyBank::SaveToFile() {                                                                                                               
    //нужно чтобы об€зательно файл по этому пути был                                                                                             
    //иначе никуда не сохранитьс€                                                                                                               
    //так сделано не потому что € ленивый                                                                                                    
    //а потому что это не мне нужно сохран€ть куда то                                                                                
    //а клиенту :)                                                                                                                          
    ofstream out;// поток дл€ записи                                                                                                          
    out.open("E:\\Test\\test.txt"); // окрываем файл дл€ записи                                                                              
    if (out.is_open()) {                                                                                                                  
        for (int i = 0; i <= N; i++) {                                                                                                   
            if(CateTop[i].empty())                                                                                                        
        ...
        }out << "ѕодписывайтесь на инстаграм*(запрещенна€ в рф соцсеть) dweavp!!!";
    }
    out.close();
}
void MyBank::Exit() {
